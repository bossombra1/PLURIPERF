...
export const StudentForumPage: React.FC<{ lang?: 'fr' | 'en' }> = ({ lang = 'fr' }) => {
  const { user } = useAuth();
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="flex items-center justify-between">
          <div className="space-y-2">
            ...
            <h1 className="text-3xl font-bold text-text-primary">Forum</h1>
          </div>
        ...
        <div className="p-8 bg-surface border border-border-subtle rounded-[calc(var(--radius)+4px)] text-center">
          <MessagesSquare className="h-12 w-12 text-text-muted mx-auto mb-4" />
          <p className="text-text-muted">Le forum sera bientôt disponible.</p>
        </div>
      </div>
    </div>
  );
