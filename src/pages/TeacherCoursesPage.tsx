...
export const TeacherCoursesPage: React.FC<{ lang?: 'fr' | 'en' }> = ({ lang = 'fr' }) => {
  const { user } = useAuth();
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="flex items-center justify-between">
          <div className="space-y-2">
            ...
            <h1 className="text-3xl font-bold text-text-primary">Mes cours</h1>
          </div>
        ...
        <div className="p-8 bg-surface border border-border-subtle rounded-[calc(var(--radius)+4px)] text-center">
          ...
          <p className="text-text-muted">Aucun cours pour le moment.</p>
        </div>
      </div>
    </div>
  );
